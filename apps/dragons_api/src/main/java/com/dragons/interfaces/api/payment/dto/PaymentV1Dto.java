...
public class PaymentV1Dto {
  ...
  public static class Card {
    ...
    public record Request(
        // 16 digits
        @Schema(description = "카드 번호(16자리)", example = "1234123412341234") @NotBlank @Pattern(regexp = "\\d{16}", message = "카드 번호는 16자리를 입력해야 합니다.") @LuhnCheck(message = "카드 번호가 유효하지 않습니다.") String cardNumber,

        // month 1-12
        @Schema(description = "만료 월(1~12)", example = "12") @Min(1) @Max(12) int expiryMonth,

        // 4-digit year
        @Schema(description = "만료 연도(4자리)", example = "2025") @Min(2000) @Max(9999) int expiryYear,

        // keep as String to allow leading zeros
        @Schema(description = "CVC(3자리)", example = "123") @NotBlank @Pattern(regexp = "\\d{3}", message = "cvc번호는 3자리여야 합니다.") String cvc,
        ...
        @Schema(description = "요금제/플랜 타입", example = "premium") @Pattern(regexp = "^(premium|basic)$", message = "유효한 플랜 타입이 아닙니다.") String planType) {
      public Request {
        if (planType == null) {
          planType = "premium";
        }
  ...
  @Schema(name = "PaymentToss", description = "토스 페이먼츠 DTO")
  public static class Toss {
    ...
    public record Request(
        ...
        @Schema(description = "요금제/플랜 타입", example = "premium") @Pattern(regexp = "^(premium|basic)$", message = "유효한 플랜 타입이 아닙니다.") String planType // Optional
    ) {
      public Request {
        if (planType == null) {
          planType = "premium";
      ...
      public PaymentPgCommand toCommand(String email) {
    ...
    @Schema(name = "PaymentTossResponse", description = "토스 페이먼츠 결제 응답")
    public record Response(
        ...
        @Schema(description = "구매자명", example = "홍길동") String customerName,

        @Schema(description = "성공 URL", example = "http://localhost:8080/api/payments/toss/success") String successUrl,

        @Schema(description = "실패 URL", example = "http://localhost:8080/api/payments/toss/fail") String failUrl) {
    }
  }
}
